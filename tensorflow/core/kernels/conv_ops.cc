 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    for (int i = DML_TENSOR_DIMENSION_COUNT_NCHW - 1; i >= 0; i--) {
      ...
      val_stride_dml_tensor_desc *= dml_filter_desc.sizes[i];
    }
    const UINT dml_tensor_desc_sizes[5] = {
        dml_filter_desc.sizes[3], dml_filter_desc.sizes[2],
        dml_filter_desc.sizes[0], dml_filter_desc.sizes[1]};
    const UINT dml_tensor_desc_strides[5] = {
        dml_filter_desc.strides[3], dml_filter_desc.strides[2],
        dml_filter_desc.strides[0], dml_filter_desc.strides[1]};
    for (int i = DML_TENSOR_DIMENSION_COUNT_NCHW - 1; i >= 0; i--) {
      dml_filter_desc.sizes[i] = dml_tensor_desc_sizes[i];
      dml_filter_desc.strides[i] = dml_tensor_desc_strides[i];
    }
    ...
    const UINT end_padding[4] = {0, 0, pad_rows, pad_cols};
