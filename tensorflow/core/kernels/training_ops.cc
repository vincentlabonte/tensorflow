...
//    const auto update =
//        (accum_update + epsilon()).sqrt() * (accum + epsilon()).rsqrt() *
//        grad;
...
//                      (prox_var.abs() - learning_rate *
//                      prox_var.constant(l1()))
//                          .cwiseMax(T(0.0)) /
//                      (var.constant(1.0) + var.constant(l2()) *
//                      learning_rate);
//    } else {
//      var.device(d) =
...
//    auto grad_with_shrinkage = grad + static_cast<T>(2) * l2_shrinkage() *
//    var; auto new_accum = accum + grad_with_shrinkage.square();
//    // special case for which lr_power=-0.5.
//    if (lr_power() == static_cast<T>(-0.5)) {
//      linear.device(d) +=
//          grad_with_shrinkage - (new_accum.sqrt() - accum.sqrt()) / lr() *
//          var;
//    } else {
//      linear.device(d) +=
//          grad_with_shrinkage -
...
//    // special case for which lr_power=-0.5.
//    if (lr_power() == static_cast<T>(-0.5)) {
//      linear.device(d) += grad - (new_accum.sqrt() - accum.sqrt()) / lr() *
//      var;
//    } else {
//      linear.device(d) +=
//          grad -
...
//    Tensor accum_update;
//    OP_REQUIRES_OK(ctx, GetInputTensorFromVariable<Device, T>(
//                            ctx, 2, use_exclusive_lock_, false,
//                            &accum_update));
//
//    OP_REQUIRES(
//        ctx, var.IsInitialized(),
...
//    Tensor accum_update;
//    OP_REQUIRES_OK(ctx, GetInputTensorFromVariable<CPUDevice, T>(
//                            ctx, 2, use_exclusive_lock_, true,
//                            &accum_update));
//    OP_REQUIRES(
//        ctx, var.IsInitialized(),
//        errors::FailedPrecondition(
...
//        errors::InvalidArgument("var and accum_grad do not have the same
//        shape",
//                                var.shape().DebugString(), " ",
...
//                errors::InvalidArgument("var must be at least 1
//                dimensional"));
//
//    const Tensor& lr = ctx->input(3);
//    OP_REQUIRES(ctx, TensorShapeUtils::IsScalar(lr.shape()),
...
//        l1.scalar<T>(), l2.scalar<T>(), grad.flat<T>());
//
//    MaybeForwardRefInputToRefOutput(ctx, 0, 0);
//  }
//
...
//
// template <typename T>
// inline T FtrlCompute(const T& accum, const T& linear, const T& lr, const T&
// l1,
...
//
//  void Compute(OpKernelContext* ctx) override {
//    auto locks = MaybeLockVariableInputMutexesInOrder(ctx,
//    use_exclusive_lock_,
//                                                      {0, 1, 2});
//    Tensor var;
//    OP_REQUIRES_OK(ctx, GetInputTensorFromVariable<Device, T>(
...
//    OP_REQUIRES_OK(
//        ctx, GetInputTensorFromVariable<Device, T>(ctx, 1,
//        use_exclusive_lock_,
...
//
//// Note, this op works on cpu only.
// template <typename Device, typename T, typename Tindex, bool
// has_l2_shrinkage>  class SparseApplyFtrlOp : public OpKernel {
// public:
...
//          T sigma = pow(updated_a, -lr_power_scalar) - pow(a,
//          -lr_power_scalar); sigma /= lr_scalar; T updated_l = l + g - sigma *
//          v; v = FtrlCompute(updated_a, updated_l, lr_scalar, l1_scalar,
//          l2_scalar,
//                          lr_power_scalar);
//          a = updated_a;
//          l = updated_l;
...
//
//    const Device& device = ctx->template eigen_device<Device>();
//    functor::ApplyMomentum<Device, T>()(device, var.flat<T>(),
//    accum.flat<T>(),
//                                        lr.scalar<T>(), grad.flat<T>(),
...
//                errors::InvalidArgument("var and m do not have the same
//                shape",
//                                        var.shape().DebugString(), " ",
//                                        m.shape().DebugString()));
//    OP_REQUIRES(ctx, var.shape().IsSameSize(v.shape()),
//                errors::InvalidArgument("var and v do not have the same
//                shape",
//                                        var.shape().DebugString(), " ",
//                                        v.shape().DebugString()));
//    OP_REQUIRES(
...
//                errors::InvalidArgument("var and ms do not have the same
//                shape",
//                                        var.shape().DebugString(), " ",
...
//                errors::InvalidArgument("var and mg do not have the same
//                shape",
//                                        var.shape().DebugString(), " ",
//                                        ms.shape().DebugString()));
//
...
//          errors::InvalidArgument("var and grad must match in dimension ",
//          d));
//    }
//    const Tindex N = indices.dim_size(0);
//    OP_REQUIRES(
...
//        device, var.flat<T>(), m.flat<T>(), lr.scalar<T>(),
//        logbase.scalar<T>(), sign_decay.scalar<T>(), beta.scalar<T>(),
//        grad.flat<T>());
//    MaybeForwardRefInputToRefOutput(ctx, 0, 0);
//  }
//
